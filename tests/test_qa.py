...
def test_catalogue_qa_includes_every_exact_record_in_grounded_prompt() -> None:
    ...
    assert "Never describe a Reserved or Sold item as available to buy" in messages[0]["content"]
    ...
    assert '"status": "Reserved"' in messages[0]["content"]
    assert '"specs": [' in messages[0]["content"]
    assert '"label": "Build volume"' in messages[0]["content"]
    assert '"title": "Hakko FX-888D Soldering Station"' in messages[0]["content"]
    assert '"title": "Bosch Router with Compact Table"' in messages[0]["content"]
...
def test_listing_prompt_injection_remains_data_inside_grounded_context() -> None:
    injected_listing = load_listings()[0].model_copy(
        update={
    ...
    system_message = completer.calls[0][0]["content"]
...
def test_qa_api_returns_model_answer_and_catalogue_sources() -> None:
    listings = load_listings()
    result = QAResult(
